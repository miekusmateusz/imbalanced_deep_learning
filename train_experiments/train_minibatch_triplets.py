...
from losses.batch_all_triplet_loss import BatchAllTtripletLoss
...
def train_minibatch_triplets(config_file):
    ...
    train_dataset_loader = DataLoader(train_dataset, batch_sampler=train_batch_sampler)
    test_dataset_loader = DataLoader(test_dataset, batch_sampler=test_batch_sampler)
    ...
    if configuration['model_params']['load_params'] == "True":
        preload = torch.load(os.path.join(configuration['model_params']['saved_model_path']))

        for key in list(preload.keys()):
            preload[key.replace('embedding_net.embedding_net', 'embedding_net')] = preload.pop(key)
        model.load_state_dict(preload)
    ...
    for epoch in range(starting_epoch, num_epochs + 1):
        ...
        model.train()
        for batch_idx, (data, labels) in enumerate(train_dataset_loader):
            ...
            criterion = BatchAllTtripletLoss(weights=train_weights, configuration=configuration)
        ...
        with torch.no_grad():
            for data, target in test_dataset_loader:
                ...
                targets = (target,)
                criterion = BatchAllTtripletLoss(configuration=configuration, weights=test_weights)
                loss_outputs = criterion(*loss_inputs, *targets)
        ...
        if epoch % epoch_log_interval == 0:
            ...
            acc, gmean, f1, cfm, n_classes_ = stratifiedKCrossVKnn(configuration, embeddings_train, e_labels_train)
            ...
            acc, gmean, f1, cfm, n_classes_ = stratifiedKCrossVKnn(configuration, embeddings_test, e_labels_test)
    ...
    knn_acc, knn_gmean, knn_f1, _, _ = stratifiedKCrossVKnn(configuration, embeddings_train,
                                                            e_labels_train)
    tree_acc, tree_gmean, tree_f1, _, _ = stratifiedKCrossVDecisionTree(configuration,
                                                                        embeddings_train,
                                                                        e_labels_train)
    _, _, _, knn_cfm, knn_n_classes_ = test_knn_classifier(configuration, embeddings_train,
                                                           e_labels_train)
